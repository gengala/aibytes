...
class GeneralizedMonarchMatrix(nn.Module):
    ...
    def __init__(self, in_dim: Tuple[int, int], out_dim: Tuple[int, int], permute_L: bool = True):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.permute_L:
            ...
            x = torch.einsum("nij,...nj->...ni", self.L, x).flatten(-2)
        return x
    ...
    def build_monarch_matrix(self):
        ...
        out = torch.block_diag(*self.L) @ PR @ torch.block_diag(*self.R)
        if self.permute_L:
    ...
    def approx(self, matrix: torch.Tensor):
        ...
        if self.permute_L:
            ...
            matrix_view = matrix.view(self.out_dim[0], self.out_dim[1], self.in_dim[1], self.in_dim[0])
...
def __main__():
    in_dim = (8, 32)
    out_dim = (64, 128)
    permute_L = True # permute_L can also be False
    mm = GeneralizedMonarchMatrix(in_dim=in_dim, out_dim=out_dim, permute_L=permute_L)
    batch_size = 1024
    ...
    mm2 = GeneralizedMonarchMatrix(in_dim=in_dim, out_dim=out_dim, permute_L=permute_L)
    out2 = mm2(x)
    print(torch.allclose(out, out2, atol=1e-4)) # False, obv
    mm2.approx(mm_materialized)
    out2 = mm2(x)
    print(torch.allclose(out, out2, atol=1e-3)) # True
